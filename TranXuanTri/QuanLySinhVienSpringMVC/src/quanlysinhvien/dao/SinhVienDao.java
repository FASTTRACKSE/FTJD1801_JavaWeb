...
public class SinhVienDao {
	...
	public int save(SinhVien sv) {
		String sql = "insert into sinhvien(hoTen,namSinh,email,gioiTinh,diaChi,lopHoc) " + "values('" + sv.getHoTen()
				+ "'," + sv.getNamSinh() + ",'" + sv.getEmail() + "','" + sv.getGioiTinh() + "','" + sv.getDiaChi()
				+ "','" + sv.getLopHoc() + "')";
	...
	public int update(SinhVien sv) {
		String sql = "update sinhvien set hoTen ='" + sv.getHoTen() + "',namSinh = " + sv.getNamSinh() + ",email = '"
				...
				+ sv.getLopHoc() + "' where id ="+sv.getId()+"";
		return template.update(sql);
	}
