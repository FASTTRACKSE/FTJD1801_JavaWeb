...
public class doUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while (fileItemsIterator.hasNext()) {
				...
				if (fileItem.isFormField()) {
					...
					if (fileItem.getFieldName().equals("iDUD")) {
						id = Integer.parseInt(fileItem.getString());
					} else if (fileItem.getFieldName().equals("hoTenUD")) {
						name = fileItem.getString("UTF-8");
					} else if (fileItem.getFieldName().equals("dateUD")) {
		...
		if (!link.equals("")) {
			...
			SinhVien sinhVien = new SinhVien(id, name, namsinh, image);
			studentDAO.updateSinhVien(sinhVien);
		}
